...
class Base_Controller extends MX_Controller
{
    ...
    //added by Shawn Rhoney - allows for proper session handling when different databases are set to use active record.
    //needed for multiple database connections
    protected  function set_userdata($data_name,$data_value)
    {

        $this->load->database('default',FALSE,TRUE);//connect to default database;
        if (is_array($data_name))
        {
            $this->session->set_userdata($data_name);
        } else {
            $this->session->set_userdata($data_name,$data_value);
        }

    }

    //needed for multiple database connections
    protected  function get_userdata($data_name)
    {
