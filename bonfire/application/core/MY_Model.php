...
class MY_Model extends BF_Model {
    ...
    //fixed key issue (allow for composite keys and non interger keys)
    protected $key=array();

    protected $fields=array();

    protected $installed = FALSE; //allows for autoinstalling / safe uninstalls
     ...
     * @return mixed An object/array representing the db row, or FALSE.
     */
    public function find(array $id= array(), $return_type = 0)
    {
       //TODO Error handling

        $keyname = array_keys($this->key);
        $count = 0;
        while ($count < count($id) && count($this->key)){
            $this->db->where($keyname[$count],$id[$count]);
            $count++;
        }
        $this->set_selects();
        $query = $this->db->get($this->table);
    ...
    public function  set_installed($installed){
        if(gettype($installed) === 'boolean')
        {
    ...
    public  function set_key($key_list)
    {
    ...
    {
        foreach ($this->key as $name=>$value) $this->key[$name] = NULL;
    }
     ...
     * * If passed in, will verify the $data is not empty.
     *
     * @param array     $id   The primary_key value to match against.
     * @param array|bool $data Array of data
     * @partial_id bool   $partial_id If FALSE(Default) Full composite key is required.
     *
     * @access protected
     *
     * @return bool
     */
   /* protected function _function_check(array $ids, &$data=FALSE,$partial_id=FALSE)
    {
        // Does the model have a table set?
        if (empty($this->table))
        {
        ...
        $this->_check_key($ids,$partial_id);
   ...
   /* protected  function _check_key(array $ids, $partial=FALSE)
    {
        // Check the IDs, but only if it contains at least one ID.
        if (! empty($ids))
        {
            ...
            //initialize $this->key
            foreach ($ids as $keyname=>$value){
                //keynames are case sensitive

                //assign value only if the keyname exist
                if (array_key_exists($keyname,$this->key) === TRUE){
                    $this->key['keyname'] = $value;
                }
            }

            //make sure at least one id is set (partial key)
            if (empty($this->key))
            {
                $this->error = $this->lang->line('bf_model_invalid_id');
                $this->logit('['. get_class($this) .': '. __METHOD__ .'] '. $this->lang->line('bf_model_invalid_id'));
            ...
            //make sure the ID is not a partial key when they're not allowed.
            if ($partial === FALSE && in_array(NULL,$this->key,TRUE))
            {
                $this->error = $this->lang->line('bf_model_invalid_id');
                $this->logit('['. get_class($this) .': '. __METHOD__ .'] '. $this->lang->line('bf_model_invalid_id'));
                return FALSE;
    ...
    protected  function findby_key(array $key_values)
    {
    ...
    //add the table to the database, if it doesn't exist
    /***
     * Creates the models table if all the proper properties (table, key(s), fields) are set.
     * @return bool
     */
    public function create(){
       $this->load->dbforge();

       $keynames = array_keys($this->key);
       if (!empty($this->fields) && count($keynames)!=0 && !empty($this->table)){
       ...
       $this->error = 'Failed to create table: invalid property setting';
