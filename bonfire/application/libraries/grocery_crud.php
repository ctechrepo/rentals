...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
    ...
    public function set_self_referencing($bool)
    {
        if ($this->basic_model == NULL){
        $this->set_default_Model();
        }
        $this->basic_model->set_self_referencing($bool);
        return $this;
    }
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_upload_file_input($field_info, $value)
	{
        ...
        $input.="<span class='fileinput-button qq-upload-button manage-uploads' id='manage-upload-button-{$unique}'> Explore files</span>";
...
{
    protected $url_segment = '';//allows for additional parameters in the url.
    ...
    //added by Shawn Rhoney
    function set_url_segment($url){
        $this->url_segment = $url;
    }
    //-----------------------
    ...
    if( sizeof($ci->uri->segments) > 0 ) {
      ...
      if( $method_name == 'index' && !in_array( 'index', $state_url_array ) ) //there is a scenario that you don't have the index to your url
		...
		//addded by Shawn Rhoney
        if(!empty($this->url_segment)){
            $state_url .= '/'.$this->url_segment;
        }
        //---------------------
...
class grocery_CRUD extends grocery_CRUD_States
{
    ...
    //added by Shawn Rhoney
    function set_database($db_con){
        $this->db_con = $db_con;
        $this->load->database($db_con,'',TRUE);
    }
    //------------------------
